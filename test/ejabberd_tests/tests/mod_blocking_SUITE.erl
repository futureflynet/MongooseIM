...
%% Note: Remember that there is no lists cleaning between test cases around one group
...
user_unblocks(Unblocker, Unblockees) when is_list(Unblockees) ->
    ...
    escalus:assert(is_iq_result, AddResult),
    escalus:assert(fun is_xep191_push/3,[<<"unblock">>, UnblockeeJIDs],BlockingPush);
